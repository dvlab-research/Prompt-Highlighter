...
def eval_model(args):
    ...
    for (input_ids, image_tensor, prompt), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        with torch.inference_mode():
            output_ids = model.generate(
                ...
                max_new_tokens=128,
                logits_processor = [ProbCFGLogitsProcessor(guidance_scale=args.cfg, use_log=False)],
                use_cache=True)
